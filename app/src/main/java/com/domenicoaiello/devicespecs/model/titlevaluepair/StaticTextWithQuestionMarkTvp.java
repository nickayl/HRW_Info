package com.domenicoaiello.devicespecs.model.titlevaluepair;

import com.domenicoaiello.devicespecs.R;

/**
 * Created by Domenico on 27/10/2017.
 */

public class StaticTextWithQuestionMarkTvp extends StaticTextTitleValuePair {

    private int option = OPTIONS_NONE;
    private String textDescription;

    public StaticTextWithQuestionMarkTvp(String mTitle, String mValue) {
        super(mTitle, mValue);
    }

    public StaticTextWithQuestionMarkTvp(String mTitle, String mValue, String mDescription) {
        super(mTitle, mValue);
        textDescription = mDescription;
    }

    @Override
    public int getAssociatedLayout() {
        return R.layout.line_item_questionmark;
    }

    public void setOption(int option) {
        this.option = option;
    }

    @Override
    public int getOptions() {
        return option;
    }


    @Override
    public String getTextDescription() {
        return textDescription;
    }

    public void setTextDescription(String textDescription) {
        this.textDescription = textDescription;
    }
}
